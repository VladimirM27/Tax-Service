...
public final class MySQLQuery {
    ...
    public static final String SELECT_ALL_UNVERIFIED_REPORTS = "SELECT  idReport,  type, status,date,users.firstName,users.lastName  FROM tax.report  JOIN reporttype  " +
            "ON tax.report.idType = tax.reporttype.idreportType  JOIN users  ON tax.report.idUser = tax.users.idUsers WHERE idInspector IS null";
    ...
    public static final String UPDATE_REPORT = "UPDATE `tax`.`report` SET `status` = ?, `commentInspector` = ?,`idInspector` = ?  WHERE `idReport` = ?;";
